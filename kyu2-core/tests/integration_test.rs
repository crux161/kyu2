use kyu2_core::{KyuPipeline, WirehairDecoder, WirehairEncoder, init};
...
fn test_full_pipeline_round_trip() {
    ...
    let protected_blob = sender_pipeline
        .protect_block(original_message, stream_id, block_id)
    ...
    let num_packets_needed = (protected_blob.len() as u32).div_ceil(packet_size);
    ...
    // 3. FEC: Decode
    let mut decoder = WirehairDecoder::new(protected_blob.len() as u64, packet_size)
    ...
    let final_message = receiver_pipeline
        .restore_block(&recovered_blob, stream_id, block_id)
